...
import (
	...
	"fivi/services/comments/v1"
	"fivi/services/comments/v1/repository"
...
func (a *app) Run(ctx context.Context) {
	...
	log.Info("The comments-service starting")
	...
	// GRPC endpoints
	{
		...
		if err != nil {
			log.Fatalf("can't prepare exchanger repository: %v", err)
		}
		...
		exchangerServer := v1.New(repository, profileClient)
		if err != nil {
			log.Fatalf("can't create exchanger server: %v\n", err)
		}

		pb_comments.RegisterServiceServer(grpcServer, exchangerServer)
	...
	log.Infof("comments-service was terminated with: %v", g.Run())
}
